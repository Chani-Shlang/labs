...
//     constructor() ERC20("Bond DAI", "bDAI") Ownable(address(this)) {}
...
//         uint256 amountLeft = mulExp(collateral, wethPrice)-(borrowed);
...
//        (uint256 fee, bool success) = Math.trymul(_amount, borrowRate);
...
//         uint256 cash =totalDeposit+totalReserve;
//        // num.add(totalBorrowed).add(totalReserve);
//         return getExp(cash, totalSupply());
//     }
...
//         uint256 wethPrice = uint256(_getLatestPrice());
//         uint256 amountLeft = mulExp(amountLocked, wethPrice)-amountBorrowed;
